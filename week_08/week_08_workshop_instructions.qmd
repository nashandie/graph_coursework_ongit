...
edu_cost_table = (
    tb.groupby("education", observed=True)
    .agg(mean_total_cost=("total_cost", "mean"), number_of_patients=("education", "count"))
    .reset_index()
)
edu_cost_table
```
...
edu_cost_table.sort_values(by='mean_total_cost', ascending=False).reset_index(drop=True)
```

Which education level has the highest mean total cost of treatment?

**Graduates education level has the highest mean total cost of treatment.**
...
```{python}
# Your code here
px.bar(edu_cost_table, y="mean_total_cost", x="education",title = 'Mean Costs by Education Level')
```

Write a short interpretation of the plot.

**The Graduate patients have an average total cost of over 3000, and this may suggest that higher education levels may be associated with high health care costs**
...
age_cost_table = (
    tb.groupby("age_group", observed=True)
    .agg(mean_total_cost=("total_cost", "mean"), num_patients=("age_group", "count"))
    .reset_index()
)
age_cost_table
```
...
```{python}
# Your code here
px.bar(age_cost_table, y="mean_total_cost", x="age_group", title="Mean costs by Age group")
```

Write a short interpretation of the plot.

**The young patients in the age group 18-36 have the highest mean total cost followed by the older patients in the 58+ group. The bar chart shows that the younger and older patients are associated with higher healthcare costs**
...
```{python}
treatment_table = (
    tb.groupby("first_visit_type", observed=True)
    .agg(
        initiatin_delay_mean=("treatment_initiation_delay_(in_days)", "mean"),
        no_of_visits=("first_visit_type", "count"),
    )
    .reset_index()
)
treatment_table
...
```{python}
px.bar(treatment_table, y="initiatin_delay_mean", x="first_visit_type", title="First visit type by average treatment initiating delay(in days)")
```

**From the chart, it can be seen that the delay is a little bit longer, an average of 9.27 days in public hospitals compared to the private hospitals with an average of 8.43 days. And counting the number of visits, the private facilities had a total of 569 visits compared to 311 for public facilities.**