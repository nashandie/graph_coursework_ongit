...
```{python}
loans_data.drop(columns=['rownames'])
```
...
```{python}
demog_info = loans_data[['emp_title', 'emp_length', 'state', 'homeownership']]
demog_info
...
credit_vars = loans_data.filter(regex="credit")
credit_vars
```

```{python}
# export the subset 'credit_vars' to a CSV file.
...
```{python}
high_income = loans_data.query('annual_income > 100000')
high_income
```

```{python}
# export the subset 'high_income' to a CSV file.
high_income.to_csv('High_Income_Information.csv')
```
...
```{python}
high_risk = loans_data.query('interest_rate > 15| debt_to_income > 30 | delinq_2y < 2')
high_risk
```

```{python}
# export the subset 'high_risk to a CSV file.

high_risk.to_csv('High_Risk_Information.csv')
```
...
```{python}
prime = loans_data.query('(grade == "A" | grade == "B") & debt_to_income < 20 & delinq_2y > 2')
prime
```

```{python}

# export the subset 'prime' to a CSV file.

prime.to_csv('Prime_Borrowers.csv')
```
...
with pd.ExcelWriter('outputs/loans_subsets.xlsx') as writer:
    demog_info.to_excel(writer, sheet_name='Demographic', index=False)
    ...
    high_risk.to_excel(writer, sheet_name='High_Risk_borrowers', index=False)
